...
def detect_faces(frame, net, min_confidence):
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > min_confidence:
            ...
            # Zoom in on the detected face and save to a file
            face_img = frame[startY:endY, startX:endX]
            face_img = cv2.resize(face_img, (300, 300))
            img_name = "face_photo/face_{}.jpg".format(str(time.time()).replace('.', ''))
...
def process_frame(frame, net, min_confidence):
    ...
    confidence = detect_faces(frame, net, min_confidence)
